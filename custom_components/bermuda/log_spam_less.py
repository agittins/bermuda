...
class BermudaLogSpamLess:
    ...
    def debug(self, key: str, msg: str, *args: object, **kwargs: Any) -> None:
    ...
    def info(self, key: str, msg: str, *args: object, **kwargs: Any) -> None:
        """Send log message, if no log was issued with the same key recently."""
        newmsg = self._prep_message(key, msg)
        if newmsg is not None:
            self._logger.info(newmsg, *args, **kwargs)

    def warning(self, key: str, msg: str, *args: object, **kwargs: Any) -> None:
        """Send log message, if no log was issued with the same key recently."""
        newmsg = self._prep_message(key, msg)
        if newmsg is not None:
            self._logger.warning(newmsg, *args, **kwargs)

    def error(self, key: str, msg: str, *args: object, **kwargs: Any) -> None:
        """Send log message, if no log was issued with the same key recently."""
        newmsg = self._prep_message(key, msg)
        if newmsg is not None:
